document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchSponsors() {
      ...
      console.log(`fetch sponsors: ${JSON.stringify(sponsorList, null, 2)}`);
    ...
    async function populateSponsorList(sponsors) {
      ...
      sponsorList.innerHTML = ''; // Clear the existing list
    ...
    function handleSponsorSelection(sponsor, listItem) {
      ...
      document.getElementById('reward-pool-id').value = sponsor.rewardPoolId.toString();
    ...
    async function updateSponsor() {
      ...
      try {
        ...
        console.log(`Updating sponsor with ID: ${sponsorId}, new company name: ${newCompanyName}, new wallet: ${newWallet}, new reward pool ID: ${newRewardPoolId}`);
        const txResponse = await sponsorsContract.updateSponsor(sponsorId, newCompanyName, newWallet, newRewardPoolId);
    ...
    document.getElementById('update-sponsor').addEventListener('click', updateSponsor);
    ...
    async function deleteSponsor() {
      ...
      try {
        ...
        console.log(`Sponsor deletion transaction hash: ${txResponse.hash}`);
        await txResponse.wait();
    ...
    document.getElementById('delete-sponsor').addEventListener('click', deleteSponsor);
  
    initializeSponsorList(); // Initialize the sponsor list on page load
  });
